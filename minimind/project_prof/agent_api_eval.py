"""
Tool-use eval using an OpenAI-compatible chat API (Qwen cloud, OpenAI, etc.):
same holdout as MiniMind eval; tools passed to API; compare first tool name to gold.
Optional: one round of mock tool result + second completion (simple agent loop).
"""
...
def main() -> None:
    p = argparse.ArgumentParser(description="Tool-call holdout eval via LLM API")
    ...
    p.add_argument("--metrics_json", type=str, default="project_prof/metrics_api_agent.json")
    p.add_argument("--bad_cases_jsonl", type=str, default="")
    p.add_argument("--two_round_agent", action="store_true", help="After first tool_calls, append mock tool result and request final reply (latency x2)")
    p.add_argument("--run_name", type=str, default="api_agent")
    args = p.parse_args()
    ...
    try:
        for i, row in enumerate(rows):
            ...
            api_tools = _openai_tools_from_minimind(tools_list)
            ...
            if valid_s:
                n_valid_structure += 1
            p_name = pred_tool_name(pred_calls[0]) if pred_calls else first_name
